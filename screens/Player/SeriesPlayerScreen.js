...
export default class PlayerScreen extends React.Component {
    ...
    componentDidMount() {
        ...
        let data;
        if (this.props.navigation.state.params.episode_id !== undefined && this.props.navigation.state.params.series_id !== undefined) {
            data = {
                episode_id: this.props.navigation.state.params.episode_id,
                series_id: this.props.navigation.state.params.series_id
            }
        }else{
            data = {
                series_id: this.props.navigation.state.params.series_id
             ...
             axios.post(Config.api_link + '/api/v1/get/watch/series',data, {
                    headers: headers
                })
                .then(response => {
                    ...
                    this.setState({
                        ...
                        current_episode: response.data.data.current_episode,
                        current_season: response.data.data.current_season
                    });
    ...
    handleProgress = progress => {
        if (!this.state.stopProgress) {
            ...
            if (progress.currentTime.toFixed() >= this.state.timeRequest) {
                ...
                axios.post(Config.api_link + '/api/v1/create/watch/series/recently', {
                    current_time: progress.currentTime.toFixed(),
                    duration_time: this.state.duration,
                    episode_id: this.state.current_episode.id,
                    series_id: this.state.current_episode.series_id
                }, {headers: headers});
            }
    ...
    handleLoad = meta => {
        if(this.state.current_episode.current_time !== null) {
            this.player.seek(this.state.current_episode.current_time)
        }
    ...
    handlePlayNext = () => {
        ...
        if (checkNextKey !== undefined) {
            ...
            this.state.current_season.map((item) => {
                const eN  = parseInt(this.state.current_episode.episode_number) + 1;
                console.log(item.episode_number  + '-' + eN);
                if(item.episode_number == eN) {
                    console.log(item);
                    this.setState({currentPlay: this.state.playlist[this.state.currentItem + 1], current_episode: item});
                }
            });

            // Add subtitle
            const textArr = [];
            this.state.textTracksLength = false;
        ...
        if(this.state.showErrorComponent) {
            ...
            const textVideoList = this.state.videoTrack.map((track, index) =>
                <ListItem key={index} onPress={() => this.changeVideoTrack(index)}><Text
                    style={index == this.state.currentVideo ? styles.captionActive : styles.captionInactive}>{track.resolution}</Text></ListItem>
            );
