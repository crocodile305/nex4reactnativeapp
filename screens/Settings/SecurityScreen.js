...
export default class LoginScreen extends React.Component {
    ...
    validation(text, type) {
        ...
        if (type === 'con-password') {

            this.setState({current_password: text});

            if (text.length >= 8) {
                this.state.validate.con_password.status = true;
            } else {
                this.state.validate.con_password.status = false;
            }
            this.setState({refersh: true});
        }
    ...
    updateRequest() {
        ...
        if (this.state.validate.password.status && this.state.validate.con_password.status) {
            ...
            axios.post(Config.api_link + '/api/v1/update/profile/password', {
        ...
        return (
            <Container style={styles.parent}>
                <Header style={styles.header}>
                    ...
                    <Body style={{flex: 2}}>
                    <Text style={styles.headerText}>{lang.security}</Text>
                    </Body>
                </Header>
                <Content>
                    ...
                    <Form style={{margin: 30}}>
                        ...
                        <Item style={styles.itemInput}>
                            <Icon name={'lock'} type={'EvilIcons'}
                                  style={this.state.inputFocus === 'pass' ? inputFocus : inputInFocus}/>
                            <Input
                                style={styles.input}
                        ...
                        <Item style={styles.itemInput}>
                            <Icon name={'lock'} type={'EvilIcons'}
                                  style={this.state.inputFocus === 're-pass' ? inputFocus : inputInFocus}/>
                            <Input
                                style={styles.input}
                                placeholder={lang.re_password}
                                secureTextEntry={true}
                                onFocus={(val) => this.setState({inputFocus: 're-pass'})}
                        ...
                        <Item style={styles.itemInput}>
                            <Icon name={'lock'} type={'EvilIcons'}
                                  style={this.state.inputFocus === 'cur-pass' ? inputFocus : inputInFocus}/>
                            <Input
                                style={styles.input}
                                placeholder={lang.current_password}
                                secureTextEntry={true}
                                onFocus={(val) => this.setState({inputFocus: 'cur-pass'})}
                                onChangeText={(text) => this.validation(text, 'con-password')}/>
                        </Item>
                        {
                            this.state.validate.showValidate && !this.state.validate.con_password.status &&
                            <Item style={styles.validateMsg}>
                                <Text style={styles.errMessageText}>{this.state.validate.con_password.msg}</Text>
                            </Item>
                        }
                    </Form>
